...
EMAIL_SIGNALS = ("enviar cv", "enviá tu cv", "envia tu cv", "mandar cv", "mandá tu cv", "send resume to", "send cv to", "email your resume")
...
def detect_application_channel(description: str, url: str = "", raw_data: dict[str, Any] | None = None) -> ApplicationDetection:
    ...
    lowered = text.casefold(); candidates = extract_recruiting_emails(text)
    ...
    has_email = bool(candidates and email_signal)
...
def extract_application_instructions(text: str) -> list[str]:
    lowered = (text or "").casefold(); results = []
    for label, signals in INSTRUCTION_SIGNALS.items():
        if any(signal in lowered for signal in signals): results.append(label)
